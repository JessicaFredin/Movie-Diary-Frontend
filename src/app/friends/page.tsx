...
export default function FriendsPage() {
	...
	const filteredFriends = friends.filter((friend) => {
		if (activeTab === "Friends") return friend.isFriend;
		if (activeTab === "Discover") return !friend.isFriend;
		return true;
	});
	...
	return (
		<div className="min-h-screen bg-background text-foreground px-6 md:px-16 py-12">
			{/* Header */}
			<div className="mb-12">
			...
			<div className="flex flex-col md:flex-row gap-6 md:items-center justify-between mb-12">
				<div className="relative w-full md:w-[500px]">
					<Search className="absolute left-4 top-3.5 text-muted w-4 h-4" />
					<input
						placeholder="Search friends..."
						className="w-full bg-surface border border-border rounded-xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-accent transition"
					/>
				</div>

				<div className="flex bg-surface border border-border rounded-xl p-1 w-fit">
					{["All", "Friends", "Discover"].map((tab) => (
						<button
							key={tab}
			...
			<div className="grid md:grid-cols-3 gap-10">
				{filteredFriends.map((friend) => (
					<div
						key={friend.id}
						className="relative bg-surface border border-border rounded-2xl p-6 transition-all duration-300 hover:border-border-strong hover:shadow-xl"
					>
						{/* Top */}
						<div className="flex items-center gap-4">
							<div className="relative">
						...
						<div className="flex gap-3">
							<button className="flex-1 bg-surface-elevated hover:bg-surface-neutral transition py-3 rounded-xl text-sm">
								Visit Profile
							</button>
							...
							{!friend.isFriend ? (
								<button
									...
									className="flex items-center gap-2 px-4 py-3 rounded-xl bg-accent hover:bg-accent-hover text-white text-sm transition"
								>
									<UserPlus size={16} />
									Add
								</button>
								...
								<button
									onClick={() => toggleFriend(friend.id)}
									className="w-12 h-12 flex items-center justify-center rounded-xl border border-border text-muted transition-all duration-200 hover:border-accent hover:bg-accent/10 hover:text-accent"
								>
									<UserMinus size={16} />
								</button>
							)}
						...
						{/* Dropdown */}
						{openMenu === friend.id && (
							<div className="absolute right-6 top-16 bg-surface-dark border border-border-strong rounded-xl w-52 p-3 shadow-2xl z-50">
								<div className="flex flex-col gap-3 text-sm">
									<button className="flex items-center gap-3 hover:text-foreground text-muted">
										<Eye size={16} />
										Visit Profile
									</button>
									<button className="flex items-center gap-3 hover:text-foreground text-muted">
									...
									<button className="flex items-center gap-3 text-accent hover:text-accent-hover">
										<UserMinus size={16} />
										Remove Friend
									</button>
									<button className="flex items-center gap-3 text-accent hover:text-accent-hover">
